...
class AutoCNNLayer(nn.Module):
    def __init__(self, hidden_size, filter_size, window_sizes):
        super().__init__()
        # 修改：移除卷积层中的LayerNorm，改为在池化后添加LayerNorm
    ...
    def __init__(self, input_dim, output_dim, num_experts):
        ...
        self.experts = nn.ModuleList([
            nn.Sequential(
                ...
                nn.LayerNorm(output_dim),  # 添加层归一化
                nn.GELU()  # 替换ReLU为GELU
...
class GCM_MultiLabelModel(EsmPreTrainedModel):
    def __init__(self, config):
        ...
        # Loss
        self.loss_resistance = AsymmetricLoss(gamma_neg=1, gamma_pos=0, clip=0.05, eps=1e-8)  # 添加微小epsilon
