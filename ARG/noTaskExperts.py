...
class GCM_MultiLabelModel(EsmPreTrainedModel):
    def __init__(self, config):
        ...
        expert_dim = 256
        num_shared_experts = 4  # 增加共享专家数量以补偿任务专家的移除
        ...
        # 移除了任务特定专家
        # 不再有self.experts_arg, self.experts_mech等

        # Gating Mechanism - 每个任务使用相同的共享专家池
        self.gate_arg = GatingMechanism(combined_hidden_size, num_shared_experts)
        self.gate_mech = GatingMechanism(combined_hidden_size, num_shared_experts)
        self.gate_abc = GatingMechanism(combined_hidden_size, num_shared_experts)
        self.gate_remove = GatingMechanism(combined_hidden_size, num_shared_experts)
    ...
    def forward(
        ...
        # 任务特定门控 - 每个任务使用相同的共享专家池
        f_arg = self.gate_arg(cls, shared_out)
        f_mech = self.gate_mech(cls, shared_out)
        f_abc = self.gate_abc(cls, shared_out)
        f_remove = self.gate_remove(cls, shared_out)
