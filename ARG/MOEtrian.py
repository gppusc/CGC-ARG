...
from AutoCNN_NewMoE import GCM_MultiLabelModel
from utils_1 import compute_metrics
from Dataset import ProteinDataset
import os

# 定义基础输出目录
BASE_OUTPUT_DIR = "outputs/AutoCNN_NewMoE_outputs_2"
...
# 检查点保存函数
def save_checkpoint(state, checkpoint_dir=os.path.join(BASE_OUTPUT_DIR, "checkpoints"), filename="checkpoint.pth.tar"):
...
def train():
    ...
    # 使用基础目录下的CSV文件路径
    csv_file = os.path.join(BASE_OUTPUT_DIR, "validation_metrics.csv")
    ...
    resume = config["training"].get("resume", False)  # 从配置中获取是否恢复训练
    ...
    optimizer = AdamW(
        [
            {"params": model.esm.parameters(), "lr": float(config["training"]["learning_rate"]) * 0.1},
    ...
    scheduler = get_linear_schedule_with_warmup(
        optimizer,
        num_warmup_steps=len(train_loader) * 1,  # 1个epoch的warmup
    ...
    best_rare_f1 = -1.0  # 初始化为负值，确保第一次比较能通过
    ...
    os.makedirs(os.path.join(BASE_OUTPUT_DIR, "epoch_models"), exist_ok=True)
    os.makedirs(os.path.join(BASE_OUTPUT_DIR, "checkpoints"), exist_ok=True)
    for epoch in range(start_epoch, config["training"]["epochs"]):
        model.train()
        total_train_loss = 0.0
        print(f"\nEpoch {epoch + 1}/{config['training']['epochs']} - Training")

        # 第一阶段：冻结ESM主干（前1/3训练周期）
        if epoch < config["training"]["epochs"] // 5:
        ...
        with open(csv_file, 'a', newline='') as f:
            ...
            writer.writerow([
                ...
                rare_antibiotic_f1,  # 稀有抗生素类别的F1分数
        ...
        # +++ 保存每一轮的模型 +++
        epoch_model_dir = os.path.join(BASE_OUTPUT_DIR, "epoch_models", f"epoch_{epoch + 1}")
        model.save_pretrained(epoch_model_dir)
        print(f"💾 Saved model_1 for epoch {epoch + 1} at: {epoch_model_dir}")

        # 保存最佳模型
        if should_save:
            best_model_dir = os.path.join(BASE_OUTPUT_DIR, "best_model")
            model.save_pretrained(best_model_dir)
            print(f"💾 Saved best model_1 at: {best_model_dir}")

        # 保存最新模型
        latest_model_dir = os.path.join(BASE_OUTPUT_DIR, "latest_model")
        ...
        }, checkpoint_dir, filename=checkpoint_filename)
